...
class TopMovieSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Movie
        fields = ['id', 'big_image_path', 'logo_image_path', 'name', 'degree', 'synopsis', 'marked', ]
        depth = 1

    def get_marked(self, obj):
        marked_info = LikeDisLikeMarked.objects.filter(movie=obj)

        if marked_info.exists():
            context = marked_info[0].marked
        else:
            context = False
