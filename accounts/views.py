...
def save_profile(reqeust):
    ...
    for div in selected_html:
        section_name.append(div.attrs['aria-label'])
...
def save_data1(request):
    ...
    for path in category_href_list:
        ...
        for i in range(20):
            ...
            # 가로그림
            horizontal_image = page1[i + 30].find('img')['src']

            page_a = page1[i + 30].find_all('a')
            href = page_a[0]['href']
